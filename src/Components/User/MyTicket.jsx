...
const TicketList = () => {
  ...
  const tickets = [
    ...
    { id: 1224, subject: "New request", status: "Closed", supportBy: "Tech support", date: "13/08/21", rating: 4.5 },
    ...
    { id: 1324, subject: "Page crash", status: "Closed", supportBy: "Web Team", date: "10/09/21", rating: 3 },
    { id: 1444, subject: "Mobile app crash", status: "On hold", supportBy: "Mobile Team", date: "11/09/21", rating: 4 },
    { id: 1544, subject: "UI bug", status: "In Progress", supportBy: "Frontend Team", date: "12/09/21", rating: 2.5 },
    { id: 1644, subject: "Backend error", status: "Closed", supportBy: "Backend Team", date: "13/09/21", rating: 5 },
    { id: 1744, subject: "Login redirect", status: "In Progress", supportBy: "Security Team", date: "14/09/21", rating: 0 },
  ];

  const getStatusStyle = (status) => {
    switch (status) {
      case "In Progress": return "bg-emerald-500 text-white";
      case "On hold": return "bg-blue-600 text-white";
      ...
      if (i < fullStars) {
        ...
        stars.push(
          <div key={i} className="relative w-3.5 h-3.5">
            <Star className="w-3.5 h-3.5 text-gray-400 absolute" />
        ...
        stars.push(<Star key={i} className="w-3.5 h-3.5 text-gray-400" />);
      }
    }
  ...
  const paginatedTickets = filteredTickets.slice(startIdx, startIdx + entriesPerPage);
      ...
      <span className="text-black text-base font-normal">Entries</span>
        ...
        <div>
          <table className="w-full text-xs">
            <thead>
              <tr>
                <th className="px-3 py-2 text-left font-medium text-black">Ticket No.</th>
                <th className="px-3 py-2 text-left font-medium text-black">Subject</th>
                <th className="px-3 py-2 text-left font-medium text-black">Status</th>
                <th className="px-3 py-2 text-left font-medium text-black">Support by</th>
                <th className="px-3 py-2 text-left font-medium text-black">Date</th>
                <th className="px-3 py-2 text-left font-medium text-black">Rate</th>
              </tr>
            </thead>
            <tbody>
              {paginatedTickets.map((ticket, index) => (
                ...
                >
                  <td className="px-3 py-2 font-medium text-blue-600">{ticket.id}</td>
                  ...
                  <td className="px-3 py-2 text-black">{ticket.supportBy}</td>
        ...
        <div className="mt-3 flex items-center justify-between text-sm">
          <div className="text-black">
            Showing {startIdx + 1} to {Math.min(startIdx + entriesPerPage, filteredTickets.length)} of{" "}
            {filteredTickets.length} entries
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handlePrev} disabled={currentPage === 1} className="px-2 py-1 bg-gray-200 rounded disabled:opacity-40"><svg width="15" height="16" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M9.93724 1.03083C9.87347 0.976597 9.79772 0.93357 9.71432 0.904211C9.63092 0.874853 9.54152 0.859741 9.45122 0.859741C9.36093 0.859741 9.27152 0.874853 9.18812 0.904211C9.10473 0.93357 9.02897 0.976597 8.96521 1.03083L0.727616 8.01908C0.663689 8.07318 0.612969 8.13744 0.578362 8.20819C0.543756 8.27894 0.525942 8.35479 0.525942 8.43139C0.525942 8.50799 0.543756 8.58384 0.578362 8.65459C0.612969 8.72534 0.663689 8.7896 0.727616 8.8437L8.96521 15.832C9.09411 15.9413 9.26893 16.0027 9.45122 16.0027C9.63352 16.0027 9.80834 15.9413 9.93724 15.832C10.0661 15.7226 10.1386 15.5743 10.1386 15.4196C10.1386 15.265 10.0661 15.1167 9.93724 15.0073L2.1843 8.43139L9.93724 1.85544C10.0012 1.80135 10.0519 1.73708 10.0865 1.66633C10.1211 1.59558 10.1389 1.51974 10.1389 1.44314C10.1389 1.36654 10.1211 1.29069 10.0865 1.21994C10.0519 1.14919 10.0012 1.08493 9.93724 1.03083Z" fill="black" fill-opacity="0.91"/>
            ...
            <button onClick={handleNext} disabled={currentPage === totalPages} className="px-2 py-1 bg-gray-200 rounded disabled:opacity-40"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
